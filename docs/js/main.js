...
// Управление модальными окнами и скроллом
document.addEventListener('DOMContentLoaded', function() {
    const calculateBtn = document.getElementById('calculateBtn');
    const orderBtn = document.getElementById('orderBtn');
    const calculateModal = document.getElementById('calculateModal');
    const closeCalculateModal = document.getElementById('closeCalculateModal');
    ...
    const contactSection = document.getElementById('contacts');
    const equipmentIcons = document.querySelectorAll('.equipment-icon');
    const equipmentSelect = document.querySelector('#contactForm .form-select');
    ...
    // Кнопка "Заказать технику" - скролл к быстрой заявке
    if (orderBtn && contactSection) {
        orderBtn.addEventListener('click', function() {
            smoothScrollTo(contactSection);
        });
    }

    // Клик по иконкам техники - скролл к форме и выбор техники
    equipmentIcons.forEach(function(icon) {
        icon.addEventListener('click', function() {
            ...
            // Выбираем соответствующий тип техники в селекте
            if (equipmentSelect && equipmentType) {
                ...
                setTimeout(function() {
                    ...
                    equipmentSelect.style.boxShadow = '0 0 10px rgba(255, 107, 53, 0.5)';
                    setTimeout(function() {
                        equipmentSelect.style.boxShadow = '';
                    }, 2000);
                }, 500);
            }
        });
    ...
    // Открытие модального окна "Рассчитать стоимость"
    if (calculateBtn && calculateModal) {
    ...
    // Закрытие модального окна "Рассчитать стоимость"
    if (closeCalculateModal && calculateModal) {
    ...
    if (calculateModal) {
        calculateModal.addEventListener('click', function(e) {
            const overlay = calculateModal.querySelector('.modal__overlay');
    ...
    document.addEventListener('keydown', function(e) {
        if (e.key === 'Escape') {
            if (calculateModal && calculateModal.classList.contains('active')) {
                calculateModal.classList.remove('active');
                document.body.style.overflow = '';
            }
    ...
    // Обработка формы "Рассчитать стоимость"
    if (calculateForm) {
        calculateForm.addEventListener('submit', function(e) {
            e.preventDefault();
            
            // Здесь можно добавить отправку данных на сервер
            console.log('Форма расчета отправлена');
            
            // Закрываем модальное окно расчета
            if (calculateModal) {
                calculateModal.classList.remove('active');
            }
    ...
    // Обработка формы "Перезвоните мне"
    if (callbackForm) {
    ...
    // Обработка формы "Быстрая заявка"
    if (contactForm) {
        contactForm.addEventListener('submit', function(e) {
            e.preventDefault();
            
            // Здесь можно добавить отправку данных на сервер
            console.log('Быстрая заявка отправлена');
    ...
    // Закрытие уведомления об успехе по клику
    if (successModal) {
